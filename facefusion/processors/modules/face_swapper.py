...
# Module-level optimization instances (initialized on first use)
TEMPORAL_SMOOTHER : Optional[TemporalSmoother] = None
ENHANCED_BLENDER : Optional[EnhancedBlender] = None


# Helper functions for batch processing and performance optimization
def _is_gpu_providers() -> bool:
	providers = state_manager.get_item('execution_providers') or []
	return any(str(p).lower() in ('cuda', 'tensorrt') for p in providers)
...
def get_temporal_smoother() -> TemporalSmoother:
	...
	if TEMPORAL_SMOOTHER is None:
		TEMPORAL_SMOOTHER = TemporalSmoother(
			...
			enable_adaptive=True # Adapt to motion
		)
	return TEMPORAL_SMOOTHER


def get_enhanced_blender() -> EnhancedBlender:
	global ENHANCED_BLENDER
	if ENHANCED_BLENDER is None:
		ENHANCED_BLENDER = EnhancedBlender(
			method='multiband',              # Best quality
			enable_color_correction=True,    # Fix color warping
			pyramid_levels=5,                # Multi-scale blending
			feather_amount=15                # Smooth boundaries
		)
	return ENHANCED_BLENDER


def paste_back_enhanced(target_vision_frame : VisionFrame, crop_vision_frame : VisionFrame, crop_mask : numpy.ndarray, affine_matrix : numpy.ndarray, use_enhanced : bool = True) -> VisionFrame:
	"""
	Enhanced version of paste_back using advanced blending techniques.
	Falls back to standard paste_back if enhanced blending is disabled.
	"""
	if not use_enhanced:
		return paste_back(target_vision_frame, crop_vision_frame, crop_mask, affine_matrix)

	try:
		# Get enhanced blender
		blender = get_enhanced_blender()

		# Inverse affine transformation to paste crop back into frame
		inverse_affine_matrix = cv2.invertAffineTransform(affine_matrix)
		target_h, target_w = target_vision_frame.shape[:2]

		# Warp the crop back to original position
		paste_vision_frame = cv2.warpAffine(
			crop_vision_frame,
		...
		# Calculate center for blending (center of non-zero mask region)
		mask_indices = numpy.where(paste_mask > 0)
		if len(mask_indices[0]) > 0:
			center_y = int(numpy.mean(mask_indices[0]))
			center_x = int(numpy.mean(mask_indices[1]))
			center = (center_x, center_y)
		else:
			# Fallback to standard paste_back if mask is empty
			return paste_back(target_vision_frame, crop_vision_frame, crop_mask, affine_matrix)

		# Use enhanced blending with color correction
		result_frame = blender.blend_face(
			swapped_face=paste_vision_frame,
			target_frame=target_vision_frame,
			mask=paste_mask,
			center=center,
			landmarks=None  # Could extract from target_face if available
		)

		return result_frame

	except Exception as e:
		# Fallback to standard paste_back on any error
		logger.debug(f'Enhanced blending failed, using standard paste_back: {e}', __name__)
		return paste_back(target_vision_frame, crop_vision_frame, crop_mask, affine_matrix)
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		# CUDA OPTIMIZATION: Batch processing and TensorRT arguments
		group_processors.add_argument('--face-swapper-batching', help = 'Batch processing mode: auto (default), always, or never', default = config.get_str_value('processors', 'face_swapper_batching', 'auto'), choices = ['auto', 'always', 'never'])
		group_processors.add_argument('--face-swapper-use-trt', help = 'Use TensorRT for faster inference (requires TensorRT)', dest = 'face_swapper_use_trt', action = 'store_true', default = config.get_bool_value('processors', 'face_swapper_use_trt', 'False'))
		group_processors.add_argument('--face-swapper-trt-max-batch', help = 'Maximum batch size for TensorRT (1-64)', type = int, default = config.get_int_value('processors', 'face_swapper_trt_max_batch', '32'), choices = range(1, 65))
...
def apply_args(args : Args, apply_state_item : ApplyStateItem) -> None:
	...
	# CUDA OPTIMIZATION: Apply batch processing and TensorRT args
...
def post_process() -> None:
	...
	# CUDA Optimization: Reset temporal smoother for next video
	global TEMPORAL_SMOOTHER
	if TEMPORAL_SMOOTHER is not None:
		TEMPORAL_SMOOTHER.reset()

	# CUDA Optimization: Clear source caches
	global _SOURCE_FACE_CACHE, _SOURCE_EMBEDDING_CACHE
	with CACHE_LOCK:
		_SOURCE_FACE_CACHE.clear()
		_SOURCE_EMBEDDING_CACHE.clear()

	# CUDA Optimization: Aggressive GPU memory cleanup for better batch processing
	...
	if state_manager.get_item('video_memory_strategy') in [ 'strict', 'moderate' ]:
		...
		# Additional GPU cleanup for moderate/strict memory strategies
		if torch.cuda.is_available():
			torch.cuda.empty_cache()
...
def swap_face(source_face : Face, target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	...
	# CUDA Optimization: Apply temporal smoothing to reduce jitter
	temporal_smoother = get_temporal_smoother()
	raw_landmarks = target_face.landmark_set.get('5/68')

	# Check for scene cuts (large motion) and reset if needed
	if temporal_smoother.should_reset(raw_landmarks):
		temporal_smoother.reset()

	# Apply temporal smoothing to landmarks for jitter reduction
	smoothed_landmarks = temporal_smoother.smooth_landmarks(raw_landmarks)

	crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(temp_vision_frame, smoothed_landmarks, model_template, pixel_boost_size)
	temp_vision_frames = []
	crop_masks = []
	...
	# CUDA Optimization: Use enhanced blending for better quality and reduced color warping
	paste_vision_frame = paste_back_enhanced(temp_vision_frame, crop_vision_frame, crop_mask, affine_matrix, use_enhanced=True)
...
def process_frame(inputs : FaceSwapperInputs) -> VisionFrame:
	...
	if source_face and target_faces:
		# CUDA OPTIMIZATION: Use batch processing when multiple faces or enabled
		batching_mode = state_manager.get_item('face_swapper_batching') or 'auto'
		use_batch = (batching_mode == 'always') or (batching_mode == 'auto' and (len(target_faces) > 1 or _is_gpu_providers()))

		if use_batch:
			# Batch processing path (PERFORMANCE OPTIMIZED)
			temp_vision_frame = swap_faces_batch(source_face, target_faces, target_vision_frame, temp_vision_frame)
		else:
			# Sequential processing path (fallback for single face CPU)
